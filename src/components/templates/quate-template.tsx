    ...
    <div className="p-6 h-full" style={{...fontStyle, backgroundColor: bgColor, color: textColor}}>
      ...
      <header className="mb-6 text-center">
        ...
        <div className="flex justify-center gap-4 mt-4 text-sm" style={lightTextStyle}>
          <div className="flex items-center gap-1"><Mail size={14} /> {data.personal.email}</div>
          <div className="flex items-center gap-1"><Phone size={14} /> {data.personal.phone}</div>
          <div className="flex items-center gap-1"><MapPin size={14} /> {data.personal.location}</div>
          {data.personal.website && (
            <a href={`https://${data.personal.website}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:underline" style={{color}}><Globe size={14} /> {data.personal.website}</a>
          )}
        </div>
      </header>
      ...
      {data.personal.description && (
        <section className="mb-6">
          <h2 className="text-xl font-bold mb-2 flex items-center gap-2" style={{...sectionTitleStyle, ...fontStyle}}><User />Profile</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          {/* Experience */}
          <section className="mb-6">
            <h2 className="text-xl font-bold mb-2 flex items-center gap-2" style={{...sectionTitleStyle, ...fontStyle}}><Briefcase />Experience</h2>
        ...
        <div>
          {/* Education */}
          <section className="mb-6">
            <h2 className="text-xl font-bold mb-2 flex items-center gap-2" style={{...sectionTitleStyle, ...fontStyle}}><GraduationCap />Education</h2>
          ...
          {/* Skills */}
          <section className="mb-6">
            <h2 className="text-xl font-bold mb-2 flex items-center gap-2" style={{...sectionTitleStyle, ...fontStyle}}><Wrench />Skills</h2>
      ...
      {data.projects && data.projects.length > 0 && (
        <section>
          <h2 className="text-xl font-bold mb-2 flex items-center gap-2" style={{...sectionTitleStyle, ...fontStyle}}><Code />Projects</h2>
